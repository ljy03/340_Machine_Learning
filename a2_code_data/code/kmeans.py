...
class Kmeans:
    ...
    # Initialize error to 0
        ...
        # Iterate over each data point and its cluster assignment
        for i in range(X.shape[0]):
            # Compute the squared distance from the data point to its cluster center
            distance_sq = np.sum((X[i] - means[y[i]])**2)
            
            # Add to the total error
            total_error += distance_sq
