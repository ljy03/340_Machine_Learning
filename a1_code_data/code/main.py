...
# CPSC 340 code
from utils import handle, load_dataset, main, mode, plot_classifier, run
...
def q5_1():
    ...
    # 2. Compute quantiles
    quantile_5 = np.percentile(X, 5)
    quantile_25 = np.percentile(X, 25)
    quantile_50 = np.percentile(X, 50)
    quantile_75 = np.percentile(X, 75)
    quantile_95 = np.percentile(X, 95)

    # 3. Regions with highest and lowest mean & variance
    mean_per_region = df.mean()
    variance_per_region = df.var()

    region_highest_mean = mean_per_region.idxmax()
    region_lowest_mean = mean_per_region.idxmin()

    region_highest_variance = variance_per_region.idxmax()
    region_lowest_variance = variance_per_region.idxmin()

    # Display the results (or you can return them as needed)
    ...
    print("50%:", quantile_50)
    print("75%:", quantile_75)
    print("95%:", quantile_95)

    print("\nRegion with highest mean:", region_highest_mean)
    print("Region with lowest mean:", region_lowest_mean)
    print("Region with highest variance:", region_highest_variance)
    print("Region with lowest variance:", region_lowest_variance)
