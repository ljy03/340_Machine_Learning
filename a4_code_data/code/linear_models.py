    ...
    def fit(self, X, y):
        ...
        while not np.all(selected):
            ...
            for j in range(d):
                ...
                w, fs, gs, ws = self.optimize(w_init, X[:, selected_with_j], y)  # Use optimize instead of fit
                current_loss, _ = self.global_loss_fn.evaluate(w, X[:, selected_with_j], y)

            # Check if the current feature set gives us a lower loss
...
class LinearClassifierOneVsAll(LinearClassifier):
    ...
    def fit(self, X, y):
        ...
        for i in range(k):
            # Convert labels to {1, -1} for the i-th class
            y_binary = np.where(y == i, 1, -1).astype(np.float32)

            # Use the i-th row of W as the initial weights
            w = W[i, :]

            # Fit the model using binary labels
            w, *_ = self.optimize(w, X, y_binary)
