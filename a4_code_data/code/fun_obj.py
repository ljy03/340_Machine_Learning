...
class LogisticRegressionLossL2(LogisticRegressionLoss):
    ...
    def evaluate(self, w, X, y):
        ...
        # Evaluate the original logistic loss
        f, g = super().evaluate(w, X, y)

        # Add L2 regularization to the function value
...
class SoftmaxLoss(FunObj):
    def evaluate(self, w, X, y):
        ...
        wcXi = X @ W.T
        expwcXi = np.exp(wcXi)
        inner_sum = np.sum(expwcXi, axis = 1)
        Wyi = W[y]
        Wyixi = np.sum(Wyi*X, axis = 1)
        f = np.sum(-Wyixi + np.log(inner_sum))
        
        softmax_probs = expwcXi/ inner_sum[:,np.newaxis]
        ytmp = np.zeros((n,k))
        ytmp[np.arange(n),y] = 1
        diff = softmax_probs - ytmp
        g = X.T @ diff
        g = g.T.flatten()
